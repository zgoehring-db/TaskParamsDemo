...
# MAGIC %md
# MAGIC # Create Metadata 
# MAGIC 
# MAGIC In this notebook we consume all of our TSQL create table statements to parse and insert into a table named `rac_adventureworks_metadata.table_metadata` which is used for tracking and maintaining specific information about each of the tables we are migrating from our DW to the Lakehouse. For now this table only contains two columns (`database_name` and `table_name`) but it could potentially hold information about the type/size of cluster needed for processing or even the type of processing required. For example, maybe we add a `load_type` column the identifies if the table is append only, merge, or truncate and reload. In short, the table allows me to know all the tables and databases in the domain.    
...
# MAGIC SET var.database_name = ${var.database_name_prefix}_adventureworks_metadata;
# MAGIC SET var.user_name = $UserName ; 
# MAGIC SET var.table_location = '/users/${var.user_name}/databases/${var.database_name_prefix}_adventureworks_metadata' ;
# MAGIC CREATE DATABASE IF NOT EXISTS ${var.database_name} LOCATION ${var.table_location} ;
# MAGIC USE ${var.database_name} -- use the metadatabase by default;

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE table_metadata 
# MAGIC (
...
all_sql_list = all_sql.split("\n")
all_sql_list[2]
database_name_prefix = dbutils.widgets.get("DatabaseNamePrefix")

table_schemas = []

for l in all_sql_list:
  if 'CREATE TABLE' in l:
    table_name = l.split(".")[1].split("[")[1][:-2]
    schema_name = l.split(".")[0].split("[")[1][:-1]
    print(f"{schema_name}.{table_name}")
    spark.sql("INSERT INTO table_metadata VALUES ('{}_{}', '{}', NULL, NULL)".format(database_name_prefix, schema_name, table_name))
...
for r in data_format_sql.split(";"):
  if 'BULK INSERT' in r:
    table = r.split(".")[1].split("]")[0][1:]
    print("TABLE: {}".format(table))
    delim_arr = r.split("FIELDTERMINATOR=")[1]
    delim = delim_arr[0:6] 
    if '\t' in delim:
      delim = "'\t'" 
    else :
    ...
    encoding_list = ["DATAFILETYPE='char'", "DATAFILETYPE ='char'", "DATAFILETYPE = 'char'", "DATAFILETYPE= 'char'"]
    encoding = 'utf-8' if any(x in r for x in encoding_list) else 'utf-16'
      
    print(delim)
    
    spark.sql("""
      UPDATE table_metadata SET delim = {}, encoding = '{}' WHERE table_name = '{}'
    """.format(delim, encoding, table))
